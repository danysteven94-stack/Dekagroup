...
export function depotView() {
  ...
  ta = state.depotTab !== "list";
  tabs = `<div style="display:flex;gap:8px;margin-bottom:18px"><button class="btn${ ta ? " ghost" : "" }" data-action="set-depot-tab" data-tab="list" style="background:${ ta ? "transparent" : COLORS.full };color:${ ta ? "var(--ink)" : "#fff" };border:1px solid ${ ta ? "var(--border)" : COLORS.full }">${ icon("boxes", 15, ta ? "var(--ink)" : "#fff") } Kontenè Full</button><button class="btn${ ta ? "" : " ghost" }" data-action="set-depot-tab" data-tab="dashboard" style="background:${ ta ? COLORS.full : "transparent" };color:${ ta ? "#fff" : "var(--ink)" };border:1px solid ${ ta ? COLORS.full : "var(--border)" }">${ icon("grid", 15, ta ? "#fff" : "var(--ink)") } Tablo Kontwôl</button></div>`;
  body = ta ? depotDivisionView() : `<div class="section-head"><div><div class="eyebrow">${ e.length } kontenè full</div><h2 class="h2">Kontenè nan Depo yo</h2></div></div><div style="display:flex;flex-direction:column;gap:8px">${ n }</div>`;
  return `<div style="min-height:100vh;background:var(--bg)"><header style="background:var(--navy);padding:18px 20px;display:flex;align-items:center;gap:12px"><div class="sidebar-logo"><img src="${ LOGO_URL }" alt="Deka Group" /></div><div style="flex:1"><div style="color:#fff;font-weight:800;font-size:16px">DEKA LOG — Depo</div><div style="color:var(--steel-light);font-size:11.5px">Jesyon kontenè Full ak depo yo · ${ formatLongDate() }</div></div>${ state.lastSyncTime ? `<div style="color:var(--steel-light);font-size:10.5px;text-align:right">Dènye sinkwonizasyon<br/>${ formatTime(state.lastSyncTime) }</div>` : "" }${ accountButton("#fff") }<button class="linklike" data-action="logout" style="color:#fff">${ icon("undo", 12) } Dekonekte</button></header><main class="content" style="max-width:820px;margin:0 auto">${ tabs }${ body }</main><div style="text-align:center;font-size:10px;color:var(--muted-light);padding:20px">© ${ new Date().getFullYear() } Deka Group · v1.0</div></div>`;
}
